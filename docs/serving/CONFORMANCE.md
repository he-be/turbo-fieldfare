...
**この表は空である。**最後に入った **RSN-4 の「文脈の残り以上の `max_tokens`
は締切を作らない」** (P7) と、その前の **GEN-14** / **RSP-3 の `draft_*`**
(P6) はどれも緑になった。残っているのは表の行ではなく **§5 完了の定義の
2 つ目と 3 つ目** — pi の実セッションと OpenAI SDK を実機で通すこと — である。
